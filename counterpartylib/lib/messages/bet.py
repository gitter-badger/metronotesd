...
def cancel_bet (db, bet, status, block_index):
    ...
    util.credit(db, bet['source'], config.XMN, bet['wager_remaining'], action='recredit wager remaining', event=bet['tx_hash'])
...
def compose (db, source, feed_address, bet_type, deadline, wager_quantity,
    ...
    if util.get_balance(db, source, config.XMN) < wager_quantity:
...
def parse (db, tx, message):
    ...
    if status == 'open':
        ...
        bet_parse_cursor.execute('''SELECT * FROM balances \
                                    WHERE (address = ? AND asset = ?)''', (tx['source'], config.XMN))
    ...
    if status == 'open':
        util.debit(db, tx['source'], config.XMN, wager_quantity, action='bet', event=tx['tx_hash'])
    ...
    for tx0 in bet_matches:
        ...
        if tx0_inverse_odds > tx1_odds:
            ...
            if tx0_wager_remaining <= 0 or tx0_counterwager_remaining <= 0:
                ...
                tx0_status = 'filled'
                util.credit(db, tx0['source'], config.XMN, tx0_wager_remaining, event=tx1['tx_hash'], action='filled')
            bindings = {
            ...
            if tx1['block_index'] >= 292000 or config.TESTNET:  # Protocol change
                if tx1_wager_remaining <= 0 or tx1_counterwager_remaining <= 0:
                    # Fill order, and recredit give_remaining.
                    tx1_status = 'filled'
                    util.credit(db, tx1['source'], config.XMN, tx1_wager_remaining, event=tx1['tx_hash'], action='filled')
            # tx1
            bindings = {
